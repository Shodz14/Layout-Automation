...
# Text lines saved in a variable
#set inputFile [open diff_pair_netlist.txt r+]
set content "
*Custom Compiler Version P-2019.06 
*Thu Apr 18 00:57:52 2024
...
*******************************************************************************
.subckt practice_schem a b
xm1 d_m1 a s_m0_m1 gnd! n08_lvt l=0.014u nf=16 m=1 nfin=2 
xm0 d_m0 b s_m0_m1 gnd! n08_lvt l=0.014u nf=16 m=1 nfin=2 
.ends practice_schem

name    D G S B    n08_lvt length   no.fingers  mult    no.fins
tarqim
...
# Split the text variable into a list of lines
...
foreach line $lines {
    # prints 1 if there is a match between the first elemnt in each line
    # and prints 0 if no match to "xm***"
    
    #puts "[string match xm* [lindex $line 0]]"
    #incr i;

    if {[string match xm* [lindex $line 0]]} {
        puts "found your desired line #[incr i] $line"
        set noequal [split $line "= "]
        puts "after tandeef -->>         $noequal "
        
        # setting length, no. fingers, mult, no.fins
        dict set instances [lindex $noequal 0] [lindex $noequal 6] [lindex $noequal 7]
        #puts "length added"
        
        dict set instances [lindex $noequal 0] [lindex $noequal 8] [lindex $noequal 9]
        #puts "fingers added"
        
        dict set instances [lindex $noequal 0] [lindex $noequal 10] [lindex $noequal 11]
        #puts "mult  added"
        
        dict set instances [lindex $noequal 0] [lindex $noequal 12] [lindex $noequal 13]
        #puts "fins added"
        #dict get $instances xm0 l
    } else {
        # puts "NOT YOUR DESIRED LINE"
        continue;
    }
}

puts "\ntry exctracting the length of the NMOS: [dict get $instances xm0 l]"

set nf_xm0 [dict get $instances xm0 nf];    # 16
set mult_xm0 [dict get $instances xm0 m];   # 4
set nf_new_xm0 [expr $nf_xm0 * $mult_xm0];  # 64

puts "\nnew no fingers: $nf_new_xm0"; #64


#puts "\n remainder :[expr $nf_new_xm0 % 4]"
puts "\n remainder :[expr 19 % 4]"

set k 4
switch $k {
        0 - 1 - 2 - 15 {puts "iterator: $k is "}
        3 4 5 {puts "but here equals $k"}
        default {puts "null"}
    }